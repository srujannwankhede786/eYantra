...
class Edrone():
	...
	def __init__(self):
		...
		self.waypoint = [ [ 0.2, 0, 23], [3.5, -3.3, 23], [ -4.4, -4.6, 20.7] , [ -6.0, 6.3, 18.3], [5.0, 5.3, 16.3] ]
		...
		self.Kp = [24.45 ,35.37 ,141.1]
		self.Ki = [0.01 ,0.01 ,0.001]
		self.Kd = [35.37 ,35.37 ,41.2]
	...
	def pitch_set_pid(self, pid_tuning_pitch):
		self.Kp[1] = pid_tuning_pitch.Kp * 0.01
		self.Ki[1] = pid_tuning_pitch.Ki * 0.01
		self.Kd[1] = pid_tuning_pitch.Kd * 0.01
	
	def roll_set_pid(self, pid_tuning_roll):
		self.Kp[0] = pid_tuning_roll.Kp * 0.01
		self.Ki[0] = pid_tuning_roll.Ki * 0.01
		self.Kd[0] = pid_tuning_roll.Kd * 0.01
	...
	#	8. Add error_sum
		...
		if modulus(self.error[0]) <0.15 and modulus(self.error[1]) <0.15 and modulus(self.error[2]) <0.15 and self.i<4:
			self.i += 1
